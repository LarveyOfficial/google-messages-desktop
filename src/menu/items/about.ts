...
const productName = "Google Messages Desktop";
const localeStyle =
...
const licenseText = `<br><br>${productName} is released under the WTFPL License.`;
...
app.on("ready", () => {
  ...
  descriptionWithLocale = `Messages for web, as a desktop app. <a href="https://github.com/LarveyOfficial/google-messages-desktop">Github Repo</a><span style="${localeStyle}">${languageCode}</span>`;
});
...
export const aboutMenuItem: MenuItemConstructorOptions = {
  ...
  click: () => {
    openAboutWindow({
      ...
      copyright: `<div style="text-align: center">Copyright (c) 2023 Luis Vervaet${disclaimerText}${licenseText}</div>`,
